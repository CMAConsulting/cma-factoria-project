...
# Script para impactar scripts SQL de settings-db en PostgreSQL
# Uso: ./impact_settings_db.sh [--profile <perfil>]
# Por defecto usa el perfil 'dev'

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_DIR="$(cd "$SCRIPT_DIR/../.." && pwd)"
DB_SCRIPTS_DIR="$PROJECT_DIR/infra/database/settings-db"
...
MODULE_NAME="impact_settings_db"
LOG_MODULE_NAME="impact_settings_db"
...
PSQL_DATABASE=$(set_with_fallback "PSQL_SETTINGS_DB" "settings_db")
...
if [[ -z "${PSQL_DATABASE}" ]]; then
  handle_error "ENV_PSQL_SETTINGS_DB no está definida en el perfil $PROFILE"
fi

# Log de inicio con información del perfil
log "INFO" "=== Impactando settings-db con perfil: $PROFILE ==="
