...
# Script para impactar scripts SQL de command-db en PostgreSQL
# Uso: ./impact_command_db.sh [--profile <perfil>]
# Por defecto usa el perfil 'dev'

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_DIR="$(cd "$SCRIPT_DIR/../.." && pwd)"
DB_SCRIPTS_DIR="$PROJECT_DIR/infra/database/command-db"
...
MODULE_NAME="impact_command_db"
LOG_MODULE_NAME="impact_command_db"
...
PSQL_DATABASE=$(set_with_fallback "PSQL_COMMANDS_DB" "commands_db")
...
if [[ -z "${PSQL_DATABASE}" ]]; then
  handle_error "ENV_PSQL_COMMANDS_DB no está definida en el perfil $PROFILE"
fi

# Log de inicio con información del perfil
log "INFO" "=== Impactando command-db con perfil: $PROFILE ==="
